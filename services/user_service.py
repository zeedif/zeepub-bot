...
try:
    ...
    from sqlalchemy import Table, Column, Integer, String, DateTime, BigInteger, MetaData, text
...
def _init_with_sqlalchemy(engine: "Engine"):
    ...
    users_table = Table(
        ...
        Column("role", String(50), nullable=False),  # 'white', 'vip', 'premium', 'staff'
...
def _upsert_sa(engine, telegram_id, role, expires_at, custom_status, created_by):
    ...
    with engine.begin() as conn:
        ...
        if exists:
            # Update
            upd = users.update().where(users.c.telegram_id == telegram_id).values(**values)
            conn.execute(upd)
        else:
            # Insert
    ...
    try:
        ...
        if exists:
            ...
            cursor.execute(
                ...
                (telegram_id, role, datetime.utcnow(), expires_at, custom_status, created_by)
            )
        conn.commit()
    finally:
        conn.close()
...
def _get_user_sa(engine, telegram_id):
    ...
    with engine.connect() as conn:
        sel = sa.select(users.c.role, users.c.expires_at, users.c.custom_status).where(users.c.telegram_id == telegram_id)
...
def _get_user_sqlite(telegram_id):
    ...
    try:
        ...
        cursor.execute("SELECT role, expires_at, custom_status FROM users WHERE telegram_id = ?", (telegram_id,))
        row = cursor.fetchone()
        if row:
...
def get_effective_user(uid: int) -> Dict[str, Any]:
    ...
    if info:
        ...
        if expires_at and expires_at < datetime.utcnow():
            # Expired
            return {"role": "free", "status_label": "Expirado", "expires_at": expires_at}
    ...
    if uid in config.PREMIUM_LIST:
        return {"role": "premium", "status_label": "Premium (Legacy)", "expires_at": None}
        
    if uid in config.VIP_LIST:
        return {"role": "vip", "status_label": "VIP (Legacy)", "expires_at": None}
        
    if uid in config.WHITELIST:
        return {"role": "white", "status_label": "Patrocinador (Legacy)", "expires_at": None}
