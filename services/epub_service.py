...
async def parse_opf_from_epub(data_or_path) -> Optional[Dict[str, Any]]:
    """Extract and parse content.opf from EPUB (bytes or file path)"""
...
def _parse_opf_bytes(data: bytes) -> Dict[str, Any]:
    """Parse OPF bytes and extract relevant metadata"""
    try:
    ...
    # Title (search by localname 'title' in DC)
    for el in root.iter():
    ...
    # Series (belongs-to-collection meta)
    for el in root.iter():
    ...
    # Subjects -> genres and demographics
    subjects = []
    for el in root.iter():
    ...
    # Split into demographics vs genres by keywords
    dem_keywords = {"juvenil", "seinen", "shounen", "shoujo", "josei", "kodomomuke", 
                   "juvenile", "chicos", "chicos/shounen", "shônen", "shounen"}
    dem = []
    genres = []
    for s in subjects:
        sl = s.lower()
    ...
    # Extract synopsis/description from OPF
    for el in root.iter():
    ...
    # Identifier -> look for URL/urn:uri
    for el in root.iter():
    ...
    # Map roles to maquetadores/traductor/ilustrador
    ...
    # If no illustrator via roles, try heuristic in contributors
    ...
    # Fallback: if still empty, try contributors that look like groups
    if not out["maquetadores"]:
        for name in contributors:
            if len(name) > 1 and name not in out["maquetadores"]:
                out["maquetadores"].append(name)
        # Last resort: try id_to_name values
        if not out["maquetadores"]:
