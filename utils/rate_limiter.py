...
class RateLimitManager:
    ...
    def _cleanup_old_requests(self, rate_limit: RateLimit):
        now = time.time()
        rate_limit.requests = [req_time for req_time in rate_limit.requests if now - req_time < rate_limit.window_seconds]

    def add_limit(self, user_id: int, limit_type: RateLimitType, max_requests: int, window_seconds: int):
        self._ensure_user(user_id)
        self.limits[user_id][limit_type] = RateLimit(max_requests=max_requests, window_seconds=window_seconds, requests=[])
